...
const EmptyState = () => (
	<div className="flex flex-col items-center justify-center p-12 h-full text-center">
		...
		<p className="text-sm text-neutral-500 max-w-sm">Create your first S3 bucket to get started</p>
	</div>
);
...
const NoSearchResults = () => (
	<div className="flex flex-col items-center justify-center p-12 h-full text-center text-balance">
		...
		<p className="text-neutral-300 font-medium mb-2">No buckets matched your search.</p>
		...
		<div className="flex flex-col gap-2 p-4" aria-label="Bucket List">
			{buckets.map((bucket) => (
				<BucketCard
					...
					handleClick={() => handleBucketClick(bucket.Name, bucket.BucketRegion)}
				/>
			))}
		</div>
	);

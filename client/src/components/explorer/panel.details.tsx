...
const EmptyState = () => (
	<div className="flex flex-col items-center justify-center size-full p-8 text-center text-balance">
		...
		<p className="text-slate-300 text-xs">Select a file or image from the explorer to view its details and preview</p>
	</div>
);
		...
		>
			...
			<div className="p-6 border-b border-slate-600">
				...
				<a
					...
					href={remoteURL}
				>
					<span className="text-sm text-neutral-100 break-all flex-1">{previewURLText}</span>
			...
			{(isImage || isVideo) && (
				<div className="p-6 border-b border-slate-600">
					<div className="flex items-center gap-2 mb-4">
						...
						<h3 className="text-sm font-medium text-neutral-100">{isVideo ? "Video Preview" : "Preview"}</h3>
					</div>
				...
				<div className="p-6">
					...
					<ul className="space-y-2">
						{variants.map((variant) => (
							<ActiveVariant key={`variant-${variant.name}`} variant={variant} remoteURL={variant.presignedUrl ?? ""} />
						))}
					</ul>
				</div>
			)}

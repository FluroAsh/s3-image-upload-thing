...
export const enhanceImageColors = async (
  ...
  const { profile = '', saturation = 1.0, vibrance = 1.0, colorSpace = 'sRGB', bitDepth = 8 } = options
  ...
  try {
    ...
    const args = [inputPath, '-colorspace', colorSpace, '-depth', bitDepth.toString()]
    ...
    if (vibrance !== 1.0) {
      args.push('-channel', 'RGB', '-evaluate', 'multiply', vibrance.toString(), '+channel')
    }
    ...
    if (convertProcess.status !== 0) {
      ...
      throw new Error(`ImageMagick processing failed: ${convertProcess.stderr}`)
    }
...
export const isImageMagickAvailable = (): boolean => {
  try {
    const result = child_process.spawnSync('convert', ['-version'], { encoding: 'utf8' })

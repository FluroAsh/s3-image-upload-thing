...
export const processNefWithDarktable = async (buffer: Buffer, options: Options = { format: 'tif' }) => {
  ...
  try {
    ...
    const dtProcess = child_process.spawnSync('darktable-cli', args, { encoding: 'utf8' })

    console.log('Executing darktable-cli:', args.join(' '))
    console.log('Darktable stdout:', dtProcess.stdout)

    if (dtProcess.status !== 0) {
      console.error(`darktable-cli error || [code ${dtProcess.status}]: ${dtProcess.stderr || 'No stderr output'}`)
      throw new Error(`Failed to process NEF with darktable-cli [exit code: ${dtProcess.status}]`)
    }
...
function locateOutputFile(tempDir: string, expectedOutputPath: string) {
  console.log(`Expected output file not found at "${expectedOutputPath}", checking for alternatives...`)

  const files = fs.readdirSync(tempDir)
  console.log('Files in temp directory:', files)

  const outputFile = files.find((file) => file !== 'input.nef' && file.startsWith('output'))
  ...
  if (outputFile) {
    ...
    console.log(`✅ Successfully processed NEF (${readableSize(processedBuffer.length)})`)

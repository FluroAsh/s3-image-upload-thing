import { ImageVariants } from '@/features/image/types'
...
export const writeToDesktop = (image: ImageVariants) => {
  ...
  const baseFolderPath = path.join(desktopPath, 'image-uploads', path.parse(image.fileName).name)
  ...
  Object.entries(image.variations).forEach(([variant, { buffer }]) => {
    const variantPath = path.join(baseFolderPath, `${variant}-${path.parse(image.fileName).name}.webp`)
